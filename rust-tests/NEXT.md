...
- 已迁移来源：239/860
- 尚未迁移来源：621
- 已迁移静态 contract：618/4161
- 尚未迁移静态 contract：3543
- 完全需要动态或自定义 Tcl 分析的脚本：250
- 最近稳定提交：`1454c7a0 Expand and decentralize the Rust testsuite`
...
以下 13 个脚本无需扩展 runner、无 compiler golden、无额外本地 fixture、无未迁移活动 assertion，预计新增 158 个 compile contract。建议拆成 `cross_suite_frontend_static` 和 `cross_suite_verilog_static` 两个稳定语义模块。

### Frontend：4 个脚本，44 个 contract

- [ ] `testsuite/bsc.syntax/bh/underscore/underscore.exp` — 23：Pass×12，tagged Fail×11
- [ ] `testsuite/bsc.syntax/bsv05/attribs/attribs.exp` — 7：tagged Fail×7
- [ ] `testsuite/bsc.bsc_examples/trafficlight/trafficlight.exp` — 10：Pass×10
- [ ] `testsuite/bsc.typechecker/ctxreduce/ctxreduce.exp` — 4：Pass×4

### Verilog/codegen：9 个脚本，114 个 contract

- [ ] `testsuite/bsc.mcd/Pragmas/Pragmas.exp` — 28：Verilog Pass×9，tagged Verilog Fail×19
- [ ] `testsuite/bsc.interra/Path_Analysis/Single_Module/Single_Module.exp` — 21 tagged Verilog Fail；3 个 count=2；`MuxLogic.bsv` 有两个不同 tag contract
- [ ] `testsuite/bsc.interra/Path_Analysis/Extended_Input_Output_Path/Extended_Input_Output_Path.exp` — 9 tagged Verilog Fail；`Argument2Rdy.bsv` count=2
- [ ] `testsuite/bsc.names/portRenaming/conflicts/modarg/modarg.exp` — 18：Verilog Pass×4，tagged Verilog Fail×14
- [ ] `testsuite/bsc.names/portRenaming/conflicts/modparam/modparam.exp` — 18：Verilog Pass×4，tagged Verilog Fail×14
- [ ] `testsuite/bsc.names/portRenaming/invalidAttrs/port/port.exp` — 7 tagged Verilog Fail
- [ ] `testsuite/bsc.names/portRenaming/invalidAttrs/enable/enable.exp` — 4 tagged Verilog Fail
- [ ] `testsuite/bsc.names/portRenaming/invalidAttrs/ready/ready.exp` — 4 tagged Verilog Fail
- [ ] `testsuite/bsc.names/portRenaming/invalidAttrs/result/result.exp` — 4 tagged Verilog Fail

## 已核对的其余静态 compile 候选

严格满足当前 runner 的 compile 脚本合计只有 35 个；除上述 13 个外还剩 22 个、302 个 contract。不要为了凑批次数量放宽条件。

### Compiler golden 密集型：12 个，201 个 contract，126 个 golden

- [ ] `testsuite/bsc.typechecker/kind/kind.exp` — 44；37 golden
- [ ] `testsuite/bsc.interra/preprocessorTestcases/define/define.exp` — 33；33 golden
- [ ] `testsuite/bsc.typechecker/primtcons/primtcons.exp` — 27；6 golden；含 Verilog；一个 error count=2
- [ ] `testsuite/bsc.bugs/github/gh353/gh353.exp` — 18；9 golden；需递归 stage `Bug353_Type.bsv`、`Types.bsv`、`Types_NonNamed.bsv`
- [ ] `testsuite/bsc.typechecker/fundeps/fundeps.exp` — 17；2 golden；`StructUpdateOneDimArray.bs` 使用 `-let-gen`；一个 error count=2
- [ ] `testsuite/bsc.typechecker/foreignmodule/parameters/parameters.exp` — 12；2 golden
- [ ] `testsuite/bsc.typechecker/foreignmodule/ports/ports.exp` — 12；3 golden
- [ ] `testsuite/bsc.typechecker/kind/mismatch/mismatch.exp` — 11 tagged Fail；11 golden
- [ ] `testsuite/bsc.bugs/github/gh221/gh221.exp` — 9；5 golden
- [ ] `testsuite/bsc.interra/preprocessorTestcases/ifdef/ifdef.exp` — 6；6 golden
- [ ] `testsuite/bsc.interra/preprocessorTestcases/undef/undef.exp` — 6；6 golden
- [ ] `testsuite/bsc.syntax/bsv05/dups/dups.exp` — 6 Fail；6 golden

### 其他静态 frontend：5 个，60 个 contract

- [ ] `testsuite/bsc.syntax/bsv05/interface/interface.exp` — 28；4 个 legacy `compile_fail source TAG` 的 TAG 实际位于 options 参数，迁移前必须确认是否原样保留
- [ ] `testsuite/bsc.typechecker/assignment/assignment.exp` — 18；`StructUpdReg.bsv` 同样有可疑 legacy options；`RegStructWrite.bsv` 有 frontend 与 Verilog 两个 contract
- [ ] `testsuite/bsc.interra/messages/EBadIfcType/EBadIfcType.exp` — 5；frontend tagged Fail，options 含 `-verilog -g <top>`，不是 Verilog mode
- [ ] `testsuite/bsc.preprocessor/ifdef/ifdef.exp` — 5 Pass
- [ ] `testsuite/bsc.driver/symtab/symtab.exp` — 4 Pass；需递归 stage 8 个 `*_Wrapper.bsv` / `*_Leaf.bsv`

### 其他静态 Verilog/codegen：5 个，41 个 contract

- [ ] `testsuite/bsc.names/portRenaming/conflicts/miscellaneous/conflicts.exp` — 21 frontend tagged Fail，options=`-verilog`；count 最大 7
- [ ] `testsuite/bsc.bsv_examples/fsm/fsm.exp` — 9；frontend+Verilog；显式 module=`mkFSM`
- [ ] `testsuite/bsc.bugs/bluespec_inc/b1490/b1490.exp` — 6 Verilog Pass；固定 RTS options=`+RTS -M288M -Sstderr -RTS`
- [ ] `testsuite/bsc.evaluator/performance/performance.exp` — 3 Verilog Pass；包含 `-M384M` / `-M265M`，资源较重
- [ ] `testsuite/bsc.names/portRenaming/conflicts/readyResult/readyResult.exp` — 3 frontend tagged Fail，options=`-verilog`；`Test01.bsv` 有两个相同调用，Rust name 必须唯一且 multiplicity=2

## 已核对的静态 simulation 候选

当前确认 39 个脚本可以用现有 SimulationCase 表达。带 compile contract 的 origin 必须同步迁移 compile case，否则 alignment 会拒绝半迁移。
