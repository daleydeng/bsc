...
当前总计覆盖 **1758 个 upstream contract**：24 个 Cargo 原生 Z3 scheduler contract，以及由自定义动态 runner 执行的 1237 个 compile contract 和 497 个 simulation contract。来源覆盖为 355/860；完整的动态统计始终以 `test-alignment` 和自动生成的 [`REMAINING.md`](REMAINING.md) 为准。
...
Upstream runner 当前完整覆盖 354 个普通 `.exp` 脚本、1734 个动态 contract。Compile pipeline 展开为 1237 个独立 contract；simulation pipeline 声明 315 个 generation scenario，并展开为 497 个 backend contract。加上 24 个 Z3 scheduler contract 后，来源覆盖为 355/860，静态可识别 contract 覆盖为 1758/4672，另有 221 个脚本需要动态或自定义 Tcl 分析。
...
迁移历史见 [`MIGRATION.md`](MIGRATION.md)，已经人工核对的下一批候选与阻塞原因见 [`NEXT.md`](NEXT.md)，由 alignment 同源生成的全部剩余 505 个脚本见 [`REMAINING.md`](REMAINING.md)。
