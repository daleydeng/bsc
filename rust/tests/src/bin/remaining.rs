use bsc_rust_tests::inventory::{
    remaining_inventory, runtime_blockers, MigrationReadiness, RemainingTestScript,
    TclCommandCategory, UnsupportedTclCommand,
};
...
fn render_inventory(entries: &[RemainingTestScript]) -> String {
    ...
    output.push_str(&format!("- Blocked Test Plans: **{}**\n", entries.len()));
    output.push_str(&format!(
        "- Typed contracts in blocked origins: **{static_contracts}**\n"
    ));
    output.push_str(&format!(
    ...
    output.push_str("## Runtime blockers in complete plans\n\n");
    output.push_str("These origins are statically complete and are therefore not part of the remaining import inventory. They are tracked separately because execution still exposes a compiler or platform regression.\n\n");
    output.push_str("| Origin | Runtime blocker |\n");
    output.push_str("| --- | --- |\n");
    for (origin, reason) in runtime_blockers() {
        output.push_str(&format!("| `{origin}` | {reason} |\n"));
    }
...
mod tests {
    ...
    fn renders_ranked_candidates_and_command_leverage() {
        ...
        let rendered = render_inventory(&entries);
        assert!(rendered.contains("Blocked Test Plans: **3**"));
        assert!(rendered.contains("Typed migration candidates: **1 scripts / 2 contracts**"));
        assert!(rendered.contains("Runtime blockers in complete plans"));
