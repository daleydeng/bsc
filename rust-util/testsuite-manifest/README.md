...
The Rust runner now has a dedicated `BluesimWorkflowScenario` model for multi-generation, link-only, and ordered multi-run workflows plus stdout artifact snapshots. Alignment parses static Tcl lists without evaluation and compares each registered generation, link, run, and transfer against this IR. Eighteen end-to-end Windows migrations now cover the core shapes: `b1489.exp` for a single run and text assertions, `b1243.exp` for link-only execution, and `traffic_light_controller_separate.exp` for two generations, ordered runs, artifact snapshots, and goldens, plus seven `Library_latency` origins and `bsc.lib/sram/sram.exp` for 24 single-generation/link/run/golden workflows, and `debugging.exp`/`b1439.exp`/`b1796.exp` for six build-only workflows, plus `eq3.exp` and `parse_strings.exp` as complete mixed origins containing two additional build-only workflows, and `rdy_en_pragmas.exp` as a complete 23-contract mixed origin with one additional build-only workflow, plus `bsc.bluesim/schedule/schedule.exp` for four build-only workflows and generated C++ scheduler assertions. Repeated runs reuse the persistent build cache, while official `dumpbo`/`dumpba` checks still validate restored intermediate artifacts.
